...
export function getFilterForName(
  list?: tableDataType[]
  ...
  let filteredItems = [
    ...new Set(
      list[0]["name"]
        ? list.map((value: tableDataType) => value["name"])
        : undefined
    ),
  ];
...
const downloadStatictic = (list: tableDataType[]) => {
  ...
  message.warning(
    "Загрузка на компьютер отключена в рамках данного проекта. Данные выводятся в консоль."
  );
...
async function apiArtifacts() {
  ...
  return {
    defaultDateRange: [
      moment(new Date(gotData[0].date)),
      moment(new Date(gotData[gotData.length - 1].date)),
...
function menuTable() {
  ...
  const [evaluation, setEvaluation] = useState<countEvaluationType>({
    TP: 0,
    TN: 0,
  });
