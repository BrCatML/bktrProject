...
const ComponentContent = (props: componentContentProps): JSX.Element => {
    let contentUrl: string = props.content.includes(namePreviewImg) ? namePreviewImg : ""
    ...
    return (
        <div style={{width: "100%", textAlign: "center"}} onClick={() => infoContent(props.itemData)}>
            {contentUrl == "" ? (
                <p>Files not found.</p>
            ) : isErrorTypeContent(contentUrl) ? (
                <FileTextFilled style={{fontSize: 50}} />
            ) : (
                <img height={50} src={`/api/${props.itemData.path}/${contentUrl}`} title={contentUrl} />
            )}
        </div>
    )
}
