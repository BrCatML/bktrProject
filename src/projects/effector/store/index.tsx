...
export const counterStore = createStore(0).on(
  counterEvent,
  (newStore: number) => {
    return newStore + 1;
  }
...
export const stringCounterStore = createStore("0").on(
  stringCounterEvent,
  (newStore: string) => {
    // counterEvent()
...
export const fetchApiFx = createEffect(
  (url: string = "https://www.boredapi.com/api/activity/") =>
    fetch(url).then((req) => req.json())
);

export const actStore = createStore<{
  current: ActivityResponse | null;
  history: ActivityResponse[];
}>({ current: null, history: [] }).on(
  fetchApiFx.doneData,
  (state, result: ActivityResponse) => {
    const history: ActivityResponse[] =
      state && state.current
        ? [state.current].concat(state.history).slice(0, 10)
        : [];
...
export const fetchApiFxCat = createEffect(
  (url: string = "https://catfact.ninja/fact") =>
    fetch(url).then((req) => req.json())
);

export const factStore = createStore<{
  current: FactResponse | null;
  history: FactResponse[];
}>({ current: null, history: [] }).on(
  fetchApiFxCat.doneData,
  (state, result: FactResponse) => {
    const history: FactResponse[] =
      state && state.current
        ? [state.current].concat(state.history).slice(0, 10)
        : [];
    return { current: result, history };
